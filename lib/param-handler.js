...
var checkType = {
	required: function (param, attrs) {
				for (var i = 0; i < attrs.length; ++i) {
					if (!_.has(param, attrs[i])) return { msg: "参数缺失" }
				}
				return;
			  },
	...
	notEmpty: function (param, attrs) {
				for (var i = 0; i < paramList.length; ++i) {
					if (_.isEmpty(paramList[i])) return { msg: "参数不能为空串" }
				}
				return;
			  } 
} 
...
function initConfig() {
	var stat = fs.statSync('param-handler.conf');
	...
	var content = fs.readFileSync('param-handler.conf', 'utf-8');
...
exports.Filter = function(method, url , param) {
	return filter[url][method](param);
};
